...
func GeminiChatHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	case types.RelayFormatClaude:
		...
		if claudeResp, ok := convertResult.Value.(*dto.ClaudeResponse); ok && claudeResp != nil {
			// Anthropic clients validate the message id shape; Gemini ids do
			// not match. Normalize at the host boundary so the kit converters
			// stay deterministic for golden pins.
