...
export function OAuthProviders({
  ...
  if (status?.oidc_enabled) {
    // Prefer the operator-configured OIDC display name; otherwise fall back
    // to the tokensheep Google auto-detection so "Continue with Google" (and
    // its brand icon) still renders for the common Google OIDC case with no
    // config.
    const looksLikeGoogle = isGoogleOidc(status)
    const oidcDisplayName = status.oidc_display_name?.trim()
    let oidcLabel: string
    if (oidcDisplayName) {
      oidcLabel = t('Continue with {{name}}', { name: oidcDisplayName })
    } else if (looksLikeGoogle) {
      oidcLabel = t('Continue with Google')
    } else {
      oidcLabel = t('Continue with OIDC')
    }
    providerButtons.push({
      key: 'oidc',
      label: oidcLabel,
