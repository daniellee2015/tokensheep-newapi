...
// MapOpenAIToolChoice re-exports the shared Claude tool-choice mapper so host
// adaptors (tokensheep's RequestOpenAI2ClaudeMessage) can reuse the same
// semantics the kit converters do without importing an internal package.
func MapOpenAIToolChoice(toolChoice any, parallelToolCalls *bool) *dto.ClaudeToolChoice {
	return sharedclaude.MapOpenAIToolChoice(toolChoice, parallelToolCalls)
}
