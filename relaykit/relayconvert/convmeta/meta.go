...
type ClaudeConvertInfo struct {
	...
	// Set when a channel cannot serve native tool calls and the relay replays
	// the result as a synthetic tool_use block instead. The id is minted once
	// so every streamed delta of that block carries the same one.
