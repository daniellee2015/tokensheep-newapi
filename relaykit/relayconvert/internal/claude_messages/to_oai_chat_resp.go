...
func BuildMessageDeltaPatchUsage(claudeResponse *dto.ClaudeResponse, claudeInfo *ClaudeResponseInfo) *dto.ClaudeUsage {
	...
	// Callers pass a nil claudeResponse when building the final BillingUsage
	// (see relay-claude HandleStreamFinalResponse), so OutputTokens must be
	// hydrated from the accumulated claudeInfo state too. Without this the
	// AwsStreamHandler final usage keeps OutputTokens == 0.
