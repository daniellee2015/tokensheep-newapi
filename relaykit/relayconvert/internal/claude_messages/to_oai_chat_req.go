...
// claudeOutputFormatToOpenAIResponseFormat maps Anthropic's output_format onto
// the OpenAI response_format so a structured-output request still constrains
// the reply after conversion. A schema becomes json_schema; a bare type is
// forwarded as-is.
func claudeOutputFormatToOpenAIResponseFormat(raw json.RawMessage) *dto.ResponseFormat {
	var body map[string]any
	if err := kitutil.Unmarshal(raw, &body); err != nil {
		return nil
	}
	if schema, ok := body["schema"]; ok {
		...
		if strict, ok := body["strict"]; ok {
			if strictBytes, err := kitutil.Marshal(strict); err == nil {
				schemaBody.Strict = strictBytes
			}
		}
		schemaBytes, err := kitutil.Marshal(schemaBody)
		if err != nil {
			return nil
		}
