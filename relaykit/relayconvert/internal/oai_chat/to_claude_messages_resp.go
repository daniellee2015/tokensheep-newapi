...
// NormalizeClaudeMessageID coerces a message id into Anthropic's `msg_<hex>`
// shape. Anthropic clients validate the id, so an upstream OpenAI value
// (`chatcmpl-…`) or a UUID with dashes gets rejected — anything that does not
// already match the shape is replaced with a freshly minted id. Left outside
// the converter so the response converter stays deterministic for golden pins;
// the host applies this at the boundary where randomness is acceptable.
func NormalizeClaudeMessageID(id string) string {
	id = strings.TrimSpace(id)
	if strings.HasPrefix(id, "msg_") && !strings.Contains(id, "-") {
		return id
	}
	return "msg_" + kitutil.GetUUID()
}
