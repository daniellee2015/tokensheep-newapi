...
| --- | ---: | --- | --- |
| 400、422 | 400 | 已识别的请求问题返回本站固定分类提示；未识别的 400 保留 `Invalid request` | `invalid_request` |
| 404、405、409、410、413、415 | 400 | 按状态返回本站固定的资源、方法、冲突、过期、大小或格式提示 | `invalid_request` |
...
`service/public_request_error.go` 只根据已确认的错误特征选择本站写死的文案，
不截取或拼接上游原文。HTTP 状态及 `type` / `code` 保持原来的公开合约，
重试和禁用仍在分类前使用原始错误。普通用户日志与公开响应使用相同的安全文案。

| 已识别的问题 | 对外提示的含义 |
| --- | --- |
| 内容缺失 | 请求内容缺失或转换后为空；检查接口与消息，输入正确时凭本站 request ID 联系支持 |
| 上下文过长 | 缩短对话或减少输入，不返回上游报告的具体 token 限额 |
| 对话以 assistant 结尾不受支持 | 检查消息序列；输入有效时按 request ID 排查协议转换 |
| 内容安全拒绝 | 提示安全检查拒绝并检查内容，不暴露原始策略标识 |
| 远程文件 URL 不受支持 | 使用支持的上传或内联格式，不回显文件 URL |
| 内容类型不受支持 | 检查文本、图片和文件格式，不回显 provider、原字段路径或值 |
| 404 / 405 / 409 / 410 | 提示模型或接口不可用、方法不支持、资源冲突或过期，不暴露内部路径与拓扑 |
| 413 / 415 / 422 | 提示请求体过大、媒体格式或参数问题，不回显代理限额和内部校验信息 |

内容缺失只描述处理结果，不据此断言客户端一定提交了空请求。未知错误继续
使用通用文案，不为“更详细”而开放原文透传。最终响应仍只追加一次本站 request ID；
回归用例同时验证 OpenAI / Gemini / Claude JSON 和已开始的 SSE，确保嵌套
上游 request ID、凭据、URL、metadata 不会出现在下游。
