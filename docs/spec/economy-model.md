...
- `PrivateKey`: 由运维在**后台 -> 支付 -> Waffo Pancake** 面板粘贴 pem 内容,存 `options` 表(与上游/100b 一致的通道),不进仓库、不进 `.env`。
...
- `WaffoPancakeApplyUSDExchangeRate = true` — 站内 $ 转 USD 时按 `USDExchangeRate` 折算(与 100b 一致)
- `USDExchangeRate = 6.8` — 全局汇率,与 100b 共用同一 option
- `WaffoPancakeSurchargePercent = 0.5` — 上送 Pancake 前加 0.5% 附加费(覆盖 pancake 平台费),用户站内充值到账仍是原额,附加费只在 Pancake 结算页显示
...
- [x] ~~Pancake 手续费 0.5% 加在上送金额里~~ (完成:`WaffoPancakeSurchargePercent = 0.5`)
...
| 2026-07-06 | Pancake live 环境;私钥走上游原生通道存 DB `options` 表,后台面板粘贴 pem;仅不入 git |
