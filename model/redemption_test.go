...
// TestRedeemCreditsQuotaExactlyOnce pins the tokensheep semantics: a welcome
// code credits `quota_gift` (not `quota`), each account may redeem at most
// once, and the redemption row flips to `used` on success. The name was
// adopted from upstream but the assertions target the gift-pool path this
// fork uses instead of the paid balance upstream credits.
func TestRedeemCreditsQuotaExactlyOnce(t *testing.T) {
	...
	assert.Equal(t, 500, user.QuotaGift)
	...
	// A second redeem by the same user is rejected by the once-per-account
	// guard, so the gift pool must not grow again.
...
func TestRedeemConcurrentSingleSuccess(t *testing.T) {
	...
	var user User
	require.NoError(t, DB.First(&user, "id = ?", userId).Error)
	// Under tokensheep semantics welcome codes credit the gift pool, so the
	// exactly-once invariant is measured there, not on paid quota.
	assert.Equal(t, 300, user.QuotaGift, "gift pool must be credited exactly once")
}
