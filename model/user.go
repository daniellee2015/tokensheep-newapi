...
func (user *User) ToBaseUser() *UserBase {
	cache := &UserBase{
		Id: user.Id,
		// The cached quota is the spendable balance, so the gift pool is folded
		// in. Debit order (gift before paid) is enforced at settlement, not here.
...
func GetUserQuota(id int, fromDB bool) (quota int, err error) {
	...
	// Spendable balance is paid quota plus the gift pool, matching what
	// ToBaseUser caches. COALESCE keeps the sum defined on all three databases
...
func DecreaseUserQuotaDetailed(id int, quota int, db bool) (QuotaDebit, error) {
	return debitUserQuota(id, quota, false)
}

// DecreaseUserQuotaAllowArrears debits `quota` unconditionally, permitting the
// paid pool to go negative. Used by tiered-retry reservation and post-consume
// settlement, where a mid-request group upgrade must charge the full delta
// even when it exceeds the wallet — matching upstream's
// `df43f8015 fix(billing): settle tiered retries with final group` invariant.
// PreConsume-like paths that want to reject up front should keep using
// DecreaseUserQuotaDetailed instead.
func DecreaseUserQuotaAllowArrears(id int, quota int) (QuotaDebit, error) {
	return debitUserQuota(id, quota, true)
}

func debitUserQuota(id int, quota int, allowArrears bool) (QuotaDebit, error) {
	if quota < 0 {
		return QuotaDebit{}, errors.New("quota 不能为负数！")
	}
	debit, err := decreaseUserQuota(id, quota, allowArrears)
	if err != nil {
...
func decreaseUserQuota(id int, quota int, allowArrears bool) (QuotaDebit, error) {
	if quota <= 0 {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if !allowArrears && user.Quota < debit.Paid {
			// Wrap the sentinel so callers using errors.Is (billing_session)
			// can detect insufficient balance without string matching. Arrears
			// mode skips this check so post-consume settlement can flip the
			// paid balance negative.
			return fmt.Errorf("%w: 剩余额度: %s, 需要扣费额度: %s", ErrInsufficientUserQuota, logger.FormatQuota(user.Quota+giftAvailable), logger.FormatQuota(quota))
		}
