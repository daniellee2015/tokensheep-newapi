...
func DisableTokenByOwnerScope(id int, scope TokenOwnerScope) (*Token, error) {
	...
	if cacheErr := invalidateTokenCacheForMutation(token.Key); cacheErr != nil {
		common.SysLog("failed to invalidate token cache after owner-scope update: " + cacheErr.Error())
	}
	return token, nil
}
...
func DeleteTokenByOwnerScope(id int, scope TokenOwnerScope) error {
	...
	if cacheErr := invalidateTokenCacheForMutation(token.Key); cacheErr != nil {
		common.SysLog("failed to invalidate token cache before owner-scope delete: " + cacheErr.Error())
	}
