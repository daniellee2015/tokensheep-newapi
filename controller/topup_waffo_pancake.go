...
func getWaffoPancakePayMoney(amount int64, group string) float64 {
	...
	// TokenSheep addition #1: convert the station-side dollar amount to the
	// Pancake settlement currency (USD) via the global exchange rate. Same
	// pattern as 100b — keeps station-side quota display at $ while charging
	// Pancake in USD.
	if setting.WaffoPancakeApplyUSDExchangeRate && operation_setting.USDExchangeRate > 0 {
		payMoney = payMoney.Div(decimal.NewFromFloat(operation_setting.USDExchangeRate))
	}
	...
	if setting.WaffoPancakeSurchargePercent > 0 {
		...
		payMoney = payMoney.Mul(surchargeMul)
	}
